...
input.each do |line|
  ...
  if moves_phase
    if match = line.match(/move (\d+) from (\d+) to (\d+)/)
      ...
      stacks[to_stack-1].push(*stacks[from_stack-1][stacks[from_stack-1].length-num_moves, num_moves])
      stacks[from_stack-1] = stacks[from_stack-1][0, stacks[from_stack-1].length-num_moves]
    end
    next
  end
