...
fn main() {
    ...
    for line in &lines {
        if is_horizontal(line) {
            ...
            for point in &points {
                *hits.entry(*point).or_insert(0) += 1;
            }
        }
    } 
...
fn get_points(line: &((i32, i32), (i32, i32))) -> Vec<(i32, i32)> {
    let (point1, point2) = (line.0.clone(), line.1.clone());
    ...
    let mut point: (i32, i32) = point1.clone();

    while point != point2 {
        points.push(point);
        let new_point = (point.0 + x_delta, point.1 + y_delta);
        point = new_point;
    }
